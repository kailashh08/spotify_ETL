...
#     description='Spotify ETL process 1-day',
#     schedule_interval=dt.timedelta(minutes=50),
# )

# conn2 = BaseHook.get_connection('postgre_sql')
# engine2 = create_engine(f'postgresql://{conn2.login}:{conn2.password}@{conn2.host}:{conn2.port}/{conn2.schema}')
# print(conn2)
# print(engine2)
...
#a = pandas.DataFrame(listA,columns=['artist_name', 'artist_ID', 'total_followers', 'genres','image_url','popularity'])
#print(a)
#load_df[1].concat(Test_Artist,ignore_index = True)

#load_df[1].loc[len(load_df[1])] = Test_Artist
...
def ETL():
    ...
    # Testing Script
    # Test_Artist = pd.DataFrame({
    # 'artist_name':["Kailash"],
    # 'artist_id':['abcdefgh12345'],
    # 'total_followers':[111],
    # 'genres':['none'],
    # 'image_url':['none'],
    # 'popularity':[11]})
    # a = load_df[1]
    # a=a.append(Test_Artist,ignore_index=True)
